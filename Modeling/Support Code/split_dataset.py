...
def gather_class_files(class_path: Path):
    return sorted([p for p in class_path.iterdir() if is_image_file(p)])

def safe_place(file_path: Path, dst_dir: Path, copy=True):
...
def main(raw_dir, out_dir, train_ratio, val_ratio, test_ratio, copy_files, random_state):
    ...
    for cls_path in classes:
        ...
        files = gather_class_files(cls_path)
        n = len(files)
        if n == 0:
        ...
        for f in train_files:
            safe_place(f, out / "train" / cls_name, copy_files)
        for f in val_files:
            safe_place(f, out / "val" / cls_name, copy_files)
        for f in test_files:
            safe_place(f, out / "test" / cls_name, copy_files)
