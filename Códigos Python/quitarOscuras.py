...
def is_dark_image(img, brightnessThreshold=BRIGHTNESS_THRESHOLD, darkPercent=DARK_PERCENT):
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # brillo promedio
    avgBrightness = np.mean(gray)
    # porcentaje de pixeles oscuros
    darkPixels = np.sum(gray < brightnessThreshold) / gray.size
    return avgBrightness < brightnessThreshold and darkPixels > darkPercent
